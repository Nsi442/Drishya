...
#
# ONLY UNDER PRESSURE, and that qualification was missing. This ran on every
# invocation, including the ones where the disk was at 58% and nothing needed
# reclaiming, and it is the only copy of the API's log on an instance whose
# containers log to json-file. So a rescue wiped the record of what the API had
# just been doing, and aws/diagnose-db.sh — run minutes later to find out
# exactly that — read the empty file and reported 'application starts in 48h: 0'
